...
#include "UnrealSumo/World/Client.h"
...
void ASumoWheeledVehicle::UpdateFromSUMO() {
    ...
    if (SumoGameInstance->ArrivedNumber > 0) {
        std::vector<std::string> ArrivedVehicleList = SumoGameInstance->ArrivedList;
        std::vector<std::string>::iterator it = std::find(ArrivedVehicleList.begin(), ArrivedVehicleList.end(), VID);
        if ( it != ArrivedVehicleList.end()) {
            /* Destroy this SumoWheeledVehicle class. */
            DestroyVehicle();
            /* Deleted the destroyed vehicle in the arrived list. */
            ArrivedVehicleList.erase(it);
            SumoGameInstance->ArrivedList = ArrivedVehicleList;
