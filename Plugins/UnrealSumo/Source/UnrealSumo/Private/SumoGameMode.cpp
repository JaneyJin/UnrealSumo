...
// NOT USED
void ASumoGameMode::UpdateFromSUMOByMachineTime() {
	TimeInWorld = GetWorld()->GetTimeSeconds();

	if (SumoGameInstance->SUMOToUnrealFrameRate.NextTimeToUpdate - TimeInWorld < 0.0001) {
		SumoGameInstance->SUMOToUnrealFrameRate.NextTimeToUpdate += SumoGameInstance->SUMOToUnrealFrameRate.UpdateDeltaT;
		UpdateSimulationFromSUMO();
	}
	/*else {
